...
#
	...
	total_assets = 0.0 # refinancing costs of the bank -> tbd in environment
	...
	num_funds = 0 # number of funds in the economy
	...
	# keep track whether a fund is active or not. This variable is necessary since it is not good
	# to remove inactive funds from funds[] while looping through them...
	active = 0
	...
	def set_parameters(self, value):
		super(Fund, self).set_parameters(value)
	def get_state_variables(self):
		return self.state_variables
	def set_state_variables(self, value):
		super(Fund, self).set_state_variables(value)
	def append_parameters(self, value):
		super(Fund, self).append_parameters(value)
	def append_state_variables(self, value):
		super(Fund, self).append_state_variables(value)
    # -------------------------------------------------------------------------
	...
	def __init__(self):
		self.accounts = []  # clear transactions when bank is initialized
				# self.Q = 0.0 # Fund liquidity is reset
	...
	def check_consistency(self):
		...
		return super(Fund, self).check_consistency(assets, liabilities)
    # -------------------------------------------------------------------------
	...
	def get_account(self,  type_):
		return super(Fund, self).get_account(type_)
    # -------------------------------------------------------------------------
	...
	def get_account_num_transactions(self,  type_):
		return super(Fund, self).get_account_num_transactions(type_)
    # -------------------------------------------------------------------------
    ...
    # i.e. fund.active instead of a bit more bulky
    # fund.parameters["active"]
...
#
	...
	#-------------------------------------------------------------------------
	def get_parameters_from_file(self,  fund_filename, environment, num_funds,  time, initial_state):

		from xml.etree import ElementTree

		try:
			xmlText = open(fund_filename).read()
			element = ElementTree.XML(xmlText)
			self.identifier = element.attrib['identifier']

			"pass in state, num_funds above if you use this"
			# self.rb = state.rb
			# self.r = state.r
			# self.global_assets_under_management = environment.variable_parameters["global_assets_under_management"]
			...
			logging.error("    ERROR: %s could not be parsed",  fund_filename)
...
#
	...
	def initialize_transactions(self, state):
		...
		# on the liabilities side, funds are endowed with investment shares from households
		# numTransactions = int(round(self.assetNumber / self.num_funds))
		# if (numTransactions == 0): # we want some error message if there are two few assets in the economy
		# 	logging.info("  ERROR: number of  assets in the economy has to be at least half the number of funds")
        ...
        #
		# transfer deposits from households to funds
...
#
# 		transaction = Transaction()
# 		transaction.this_transaction("D",  -1,  self.identifier,  value,  0,  0, -1)
# 		self.accounts.append(transaction)
# 		del transaction
#
