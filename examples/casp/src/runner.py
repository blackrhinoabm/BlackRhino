...
class Runner(BaseRunner):
    ...
    def do_run(self, environment, time):
        ...
        for i in range(self.num_sweeps):
                ...
                results_a2.append(environment.funds[0].get_account("investment_shares"))
                results_b2.append(environment.funds[1].get_account("investment_shares"))
                results_c2.append(environment.funds[2].get_account("investment_shares"))
                results_d2.append(environment.funds[3].get_account("investment_shares"))
        ...
        ###############
        xmin = 0
        ax[0][0].plot(self.updater.asset_a.firm.profit_results)
        ax[0][0].plot(self.updater.asset_b.firm.profit_results)
        ax[0][0].set_xlim(xmin=xmin)
        ax[0][0].set_xlabel("period", fontsize=15)

        ax[0][0].legend(["profit a", "profit b"], loc='best')
        ...
        #
        ax[1][0].legend(["price risk free"], loc='best')
        # ax[2][0].plot(environment.assets[2].prices)
        ax[1][0].plot(results_e)
        ax[1][0].set_xlim(xmin=xmin)
        ax[1][0].legend(["yield risk free"], loc='best')

        # ax[2][0].set_xlabel("period", fontsize=15)
        # ax[2][0].legend(["price risk free"], loc='best')

        ax[1][1].plot(self.updater.asset_a.prices)
        ax[1][1].plot(self.updater.asset_b.prices, color="crimson" )
        ax[1][1].set_xlim(xmin=xmin)
        # ax[2][1].set_ylim(ymin=32)
        # ax[2][1].set_ylim(ymax=40)

        ax[1][1].set_xlabel("period", fontsize=15)
        ax[1][1].legend(["price_a", "price_b"], loc='best')
        # ax[2][1].legend(["price_b"], loc='best')

        ax[0][1].plot(environment.assets[0].riskyness)
        ax[0][1].plot(environment.assets[1].riskyness)
        ax[0][1].set_xlim(xmin=xmin)
        ax[0][1].set_xlabel("period", fontsize=15)
        ax[0][1].legend(["std_a", "std_b"], loc='best')
        ...
        ax[2][1].plot(results_a2)
        ax[2][1].plot(results_b2)
        ax[2][1].plot(results_c2)
        ax[2][1].plot(results_d2)
        #
        # ax[3][1].set_xlim(xmin=xmin)
        # ax[4][1].set_xlabel("period", fontsize=15)
        # ax[4][1].legend(["investment_shares" ], loc='best')
        #
