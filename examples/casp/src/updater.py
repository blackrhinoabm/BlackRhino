...
class Updater(BaseModel):
    ...
    def do_update(self,  environment, time, debug):
        ...
        self.global_assets_under_management = state.global_assets_under_management
        self.num_funds= state.num_funds

        #initially, calculate the assets per fund (i.e. fund size)
        #every other time step, total assets fluctuate via deposit fluctuations
        if time == 0:
            self.allocate_fund_size(environment, state)

        # fund.initialize_transactions(state)

    def allocate_fund_size(self, environment, state):

        for fund in environment.funds:
            for i in range (len(environment.funds)):
                fund.total_assets = self.divide_sum(int(self.num_funds), int(self.global_assets_under_management))[i]
            print fund.total_assets, self.num_funds
