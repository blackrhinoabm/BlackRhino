...
#
    ...
    mylist = np.random.lognormal(mu, sigma, 2).tolist()
    ...
    pd.concat(output, ignore_index = False).to_csv("test.csv")
        ...
        #
        #
        #     df_all = pd.concat(list_of_dataframes, keys=[ float(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3])], ignore_index = False).to_csv("test.csv")

    print('Program DONE! Fire-sales happend!')
    logging.info('FINISHED Program logging for run: %s \n', environment_directory + identifier + ".xml")


"The code below passes in three specific shocks\
and the asset class as arguments\
For example for bonds\
python firesale.py -0.1 -0.5 -0.4 m_14"
    ...
    # #
    #     mu, sigma = 2., 0.7 # mean and standard deviation
    #     shocks = np.random.lognormal(mu, sigma, 10)
    #
