...
def init_assets(environment, asset_directory, time, risky_assets_funda_values):
    """
    Global assets are initialised.

    Bond  from config files and price is calculated with calc_bond_price

    Calls methods in same script:
     - initialize_shares
     - init_asset_prices
     - initialize_bond_from_files
    ...
    Output
    ===
    dictionary in environment.regions

    """
    global_assets = []
    #Risky shares are different class than bonds, so we need to
    # instantialize sequentially
    ...
    for asset in list_shares:
        print asset.domicile
        for tuple in environment.region_labels:
            if  tuple[0] == asset.domicile:
                print tuple[1]
    # environment.region_labels[1]
                dict2 = {
                        tuple[1] : [asset]
                        }
        temp.update(dict2)
    ...
    for asset in list_bonds:
        for key, value in temp.iteritems():
            print environment.region_labels
            for tuple in environment.region_labels:
                if  tuple[0] == asset.domicile:
                    if key ==  tuple[1]:
                        print key, "key", asset.domicile,
                        value.append(asset)

    environment.region.update(temp)
    print environment.region, "regions!!!!"

def initialize_shares(environment, asset_directory, time):
    from src.agent_classes.asset import Asset_risky
    shares_list = []
...
def initialize_bond_from_files(environment, asset_directory):
    from src.agent_classes.asset import Asset_riskfree
...
def init_asset_prices(environment, risky_assets_funda_values):
    ...
    ###########
    # Print fundamental values and prices"
    ...
    for ii in range(xtra_values):
		...
		#Generate a random value for history:
		i.returns.append(norm1.rvs())
    print "initilisation.py: initialize history of returns", i.returns, i.identifier
    ...
    """
    from bond_price import calc_bond_price
...
def initialize_firms_from_files(environment, firm_directory, time):
    from src.agent_classes.firm import Firm
...
def init_profits(environment, time):
    """
    This is the first initialization function.
    It determines fundamental values of risky shares.
    ...
    assets_funda_values = {
                        domestic_market: {A: float , B: float  }  ,
                        foreign_market: {A: float, B:float}
                            }
    """
    "Add some variables to firms and initialize profits"
    # for firm in environment.firms:
    #      firm.add_stuff( initial_profit=[firm.initial_profit] ,growth = [0])
    """ This I started but haven't finished
    temp = {}
    for tuple in environment.region_labels:
        for firm in environment.firms:
            asset_ident = str(firm.identifier) + " stocks"
            # print asset_ident
            brown_delta = "brown_delta " + asset_ident
            name =  tuple[1]
            if tuple[0] == firm.domicile:
                print name, "hei"
                temp2 = {
                            name: {brown_delta : firm.brown_delta}
                            }
                temp.update(temp2)
        print temp
        """
#      firm.add_stuff( initial_profit=[firm.initial_profit] ,growth = [0])
    ...
    for firm in environment.firms:
        if firm.domicile == 0:
            # growth for x_a multiply with initial profit
            x_a[0] = np.multiply(x_a[0], firm.initial_profit)
        if firm.domicile == 1:
            # growth for x_b multiply with initial profit
            x_b[0] = np.multiply(x_b[0], firm.initial_profit)
    # transfer numpy array into list
    ...
    for firm in environment.firms:
        if firm.domicile == 0:
            # The fundamental value is the profit for the period divided by the number of shares times the risk adjusted discount rate
            x_a_funda[0] = np.divide(x_a_funda[0], (firm.number_of_shares * firm.discount))
            # print x_a_funda[0], firm.number_of_shares , firm.discount, "hello"
    ...
    plt.plot(profit_funda_a)
    plt.plot(profit_funda_b )
    plt.legend(["a", "b"])
    plt.show()
    ...
    # risky_assets_funda_values.update(temp)
...
def initialize_funds_from_files(environment, fund_directory, time):
    from src.agent_classes.fund import Fund
